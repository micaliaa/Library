...
const Register = () => {
   ...
   const handleChange=(e)=>{
    ...
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:3000/akun/register', formData);
      alert(response.data.message);

    const loginRes = await axios.post('http://localhost:3000/akun/login', {
      Email: formData.Email,
      Password: formData.Password
    });

    // Simpan data user ke localStorage atau context  
    ...
    <div className="flex min-h-screen bg-[#FFF9F3]">
      ...
      <div className="flex flex-col justify-center w-full md:w-1/2 px-10 md:px-20">
        ...
        <form className="space-y-5"onSubmit={handleSubmit}>
          {/* Username */}
          <div>
            <label className="block text-[#5A4A42] text-sm font-medium mb-2">
              Username
            </label>
            <input
              type="text"
      ...
      <div className="hidden md:flex w-1/2 bg-[#F5E6D3] items-center justify-center">
