...
fetchDetailUser();
    ...
    const handleLogout=()=>{
        //hapus data pengguna dari locallstorage
        localStorage.removeItem("token");
        localStorage.removeItem("UserID");
        localStorage.removeItem("Role");

        //kembali ke halaman login jika sudah logout
        navigate('/login');
    };
    if(!user){
        return null
    ...
    return(
       ...
       className='w-full mt-6 py-2 bg-[#F5E6D3] text-[#7B3F00] rounded-md font-bold hover:bg-[#D29D6A]'>Logout</button>
